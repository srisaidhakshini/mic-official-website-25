...
// Props for the TechSecCard
interface TechSecCardProps {
  name: string;        // The name to display on the card
  imageSrc?: string;   // (Optional) The image URL to display in the image area
}

/**
 * TechSecCard - Pixel-perfect card for the Tech Secretary role, matching Figma.
...
const TechSecCard: React.FC<TechSecCardProps> = ({ name, imageSrc = '/images/mic_board/techsec_mannat.jpg' }) => {
  ...
  return (
    ...
    >
      ...
      <img
        src="/images/techsec.svg"
        alt="Tech Secretary Card Background"
        width={CARD_WIDTH}
        height={CARD_HEIGHT}
      ...
      {/* NAME: Insert the Tech Secretary's name here. Update the 'name' prop to change the displayed name. */}
      <div
      ...
      {/* No need to add stars.svg here; the stars are already part of techsec.svg. */}
    </div>
  );
};
